...
Если у вас используется старая версия Docker Compose, можно запустить так:
...
После запуска подключитесь к PostgreSQL со следующими параметрами:
...
Если запрос выполняется успешно, значит база данных поднята и данные загружены.
...
Чтобы остановить контейнер и удалить данные:
